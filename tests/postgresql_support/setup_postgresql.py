...
def check_postgresql_installation():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("   💡 Start PostgreSQL with: brew services start postgresql (macOS) or systemctl start postgresql (Linux)")
            return False
            
    except (subprocess.CalledProcessError, FileNotFoundError):
        print("   ❌ PostgreSQL not found")
        print("   💡 Install with: brew install postgresql (macOS) or apt-get install postgresql (Linux)")
        return False
...
def setup_test_database():
    ...
    try:
        ...
        test_cursor.execute("SELECT name FROM pg_available_extensions WHERE name IN ('unaccent', 'pg_trgm')")
        ...
        test_cursor.execute("SELECT to_tsvector('english', 'Test full-text search capabilities')")
