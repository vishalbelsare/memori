...
def test_database_performance(db_type, connection_string, test_name):
    ...
    try:
        ...
        print(f"📊 Database info: {db_info['database_type']} v{db_info.get('server_version', 'N/A')}")
        ...
        print(f"   ⏱️  Insert time: {insert_time:.3f}s ({insert_time/10:.3f}s per record)")
        ...
        print(f"   📈 Stats: {stats['chat_history_count']} chats, {stats['database_type']} backend")
        
        # Test 3: Search functionality
        print(f"\n🔍 Test 3: Search functionality")
        start_time = time.time()
        
        search_results = memory.db_manager.search_memories("performance test database", namespace="performance_test")
        ...
        return {
            ...
            'search_strategy': search_results[0].get('search_strategy', 'none') if search_results else 'none'
        }
...
def main():
    ...
    with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tmp_db:
        ...
        sqlite_result = test_database_performance("SQLite", sqlite_connection, "SQLite Database")
        if sqlite_result:
    ...
    try:
        ...
        mysql_result = test_database_performance("MySQL", mysql_connection, "MySQL Database")
        if mysql_result:
    ...
    if len(results) >= 2:
        ...
        if sqlite_result and mysql_result:
            ...
            print(f"{'Insert (10 records)':<20} {sqlite_insert:.3f}s{'':<8} {mysql_insert:.3f}s{'':<8} {insert_winner:<10}")
            
            # Retrieval performance  
            sqlite_retrieval = sqlite_result['retrieval_time']
            mysql_retrieval = mysql_result['retrieval_time']
            retrieval_winner = "SQLite" if sqlite_retrieval < mysql_retrieval else "MySQL"
            print(f"{'Retrieval':<20} {sqlite_retrieval:.3f}s{'':<8} {mysql_retrieval:.3f}s{'':<8} {retrieval_winner:<10}")
            ...
            print(f"{'Search':<20} {sqlite_search:.3f}s{'':<8} {mysql_search:.3f}s{'':<8} {search_winner:<10}")
            ...
            print(f"{'Connections (5x)':<20} {sqlite_conn:.3f}s{'':<8} {mysql_conn:.3f}s{'':<8} {conn_winner:<10}")
            
            print(f"\n🔍 Search Strategy Comparison:")
            print(f"   SQLite: {sqlite_result['search_strategy']} ({sqlite_result['search_results']} results)")
            print(f"   MySQL:  {mysql_result['search_strategy']} ({mysql_result['search_results']} results)")
            
            print(f"\n🎯 Key Insights:")
            print(f"   • SQLite is typically faster for single-user, file-based operations")
            print(f"   • MySQL provides better concurrency and enterprise features") 
            print(f"   • Both support full-text search with different implementations")
            print(f"   • Cross-database compatibility maintained with identical APIs")
