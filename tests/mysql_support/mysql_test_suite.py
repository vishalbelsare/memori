...
def run_mysql_test_scenario(test_name, provider_config, test_inputs, database_url):
    ...
    try:
        ...
        assert db_info['database_type'] == 'mysql', f"Expected MySQL, got {db_info['database_type']}"
        ...
        history = memory.db_manager.get_chat_history(namespace="mysql_test", session_id=session_id)
        ...
        results = memory.db_manager.search_memories("MySQL testing", namespace="mysql_test")
        print(f"   📊 Search returned {len(results)} results (expected 0 for new database)")
        
        # Test memory statistics
        print("   📈 Testing memory statistics...")
        stats = memory.db_manager.get_memory_stats("mysql_test")
        expected_stats = ['chat_history_count', 'short_term_count', 'long_term_count', 'database_type']
        for stat in expected_stats:
            assert stat in stats, f"Missing statistic: {stat}"
        
        assert stats['database_type'] == 'mysql', f"Database type mismatch: {stats['database_type']}"
        assert stats['chat_history_count'] >= 1, f"Chat history count incorrect: {stats['chat_history_count']}"
        print(f"   ✅ Statistics: {stats['chat_history_count']} chats, {stats['short_term_count']} short-term, {stats['long_term_count']} long-term")
...
def run_mysql_performance_test(database_url):
    ...
    try:
        ...
        history = memory.db_manager.get_chat_history(namespace="mysql_perf_test", limit=20)
        assert len(history) >= 10, f"Expected at least 10 records, got {len(history)}"
        
        # Test search performance
        results = memory.db_manager.search_memories("performance test", namespace="mysql_perf_test")
        
        elapsed_time = time.time() - start_time
        print(f"   ✅ Performance test completed in {elapsed_time:.2f}s")
        print(f"   📊 Processed 10 inserts, retrieved {len(history)} records, found {len(results)} search results")
...
def main():
    ...
    if not mysql_available:
        ...
        print("   2. mysql-connector-python is installed: pip install mysql-connector-python")
        print("   3. MySQL user 'root' has access (adjust credentials in script if needed)")
        return 1
    ...
    if passed_tests == total_tests:
        print("🎉 All MySQL tests passed! Cross-database compatibility is working correctly.")
        return 0
    else:
