...
def run_mysql_test_scenario(test_name, conscious_ingest, auto_ingest, test_inputs):
    ...
    init_kwargs = {
        "database_connect": mysql_connection,
        "verbose": True
    }
    ...
    print(f"✅ Memori initialized with MySQL backend")
...
def main():
    ...
    print(f"🗃️  Testing with MySQL backend")
    ...
    if len(all_results) > 0:
        ...
        print(f"{'Scenario':<35} {'Success':<8} {'Failed':<8} {'Chats':<8} {'Duration':<10}")
        print("-" * 70)
        ...
        for result in all_results:
            ...
            print(f"{result['scenario']:<35} {result['successful_calls']:<8} "
                  f"{result['failed_calls']:<8} {result['chats_added']:<8} "
                  f"{result['duration']:.1f}s{'':<5}")
        
        print("-" * 70)
        print(f"{'TOTALS':<35} {total_success:<8} {total_failed:<8} {total_chats:<8} {total_duration:.1f}s")
        ...
        print(f"   • MySQL FULLTEXT search is working across all scenarios")
        
        # Database backend confirmation
        print(f"\n🗄️  Database Backend Confirmation:")
        print(f"   • All scenarios used MySQL backend successfully")
