...
from memoriai.utils.exceptions import DatabaseConnectionError, DatabaseInitializationError
...
class TestDatabaseManager:
    ...
    def test_initialize_database_connection_error(self, mock_create_engine, memory_db_settings):
    ...
    def test_backup_database(self, temp_db_settings):
        ...
        manager.execute_query(
            "INSERT INTO test_backup (data) VALUES (?)",
            ("test data",)
        )
    ...
    def test_restore_database(self, temp_db_settings):
        ...
        source_manager.execute_query(
            "INSERT INTO test_restore (data) VALUES (?)",
            ("restore test data",)
        )
        ...
        try:
            ...
            source_path = temp_db_settings.database.connection_string.replace("sqlite:///", "")
    ...
    def test_transaction_rollback(self, db_manager):
        ...
        try:
            with db_manager.get_session() as session:
                # Insert first record
                session.execute(
                    "INSERT INTO transaction_test (data) VALUES (?)",
                    ("unique_data",)
                )
