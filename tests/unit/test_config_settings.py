...
class TestMemoriSettings:
    ...
    def test_memori_settings_from_env_variables(self):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            assert settings.database.connection_string == "postgresql://env:test@localhost/env"
            assert settings.database.echo is True
            assert settings.agents.default_model == "gpt-4"
    ...
    @patch("builtins.open", new_callable=mock_open, read_data='{"database": {"connection_string": "file://test"}}')
    ...
    def test_memori_settings_validation_errors(self):
        ...
        with pytest.raises(ValidationError):
            MemoriSettings(
                database=DatabaseSettings(pool_size=-1)  # Invalid pool size
            )
    ...
    def test_memori_settings_partial_updates(self):
        ...
        assert updated_settings.memory.default_namespace == settings.memory.default_namespace