...
def get_database_connections():
    ...
    try:
        ...
        connections['PostgreSQL'] = f"postgresql+psycopg2://{user}{password_part}@{host}:{port}/memori_test"
    except:
        pass
...
def test_database_comprehensive(db_name, connection_string, test_name):
    ...
    try:
        ...
        print(f"📊 Database: {db_info['database_type']} v{db_info.get('server_version', 'N/A')}")
        ...
        print(f"   ⏱️  Insert time: {test_results['insert_time']:.3f}s ({test_results['insert_time']/len(test_messages):.3f}s per record)")
        ...
        for query in search_queries:
            ...
            if results:
                ...
                print(f"   🔎 '{query[:30]}...': {len(results)} results in {query_time:.3f}s ({strategy})")
        
        test_results['search_time'] = total_search_time
        test_results['search_results'] = total_results
        test_results['search_strategy'] = list(search_strategies)[0] if search_strategies else 'none'
        ...
        try:
            ...
            print(f"   ✅ Short-term memory: Success")
            ...
            print(f"   ✅ Long-term memory: Success")
...
def main():
    ...
    if len(all_results) >= 2:
        print(f"\n{'=' * 70}")
        print(f"📊 COMPREHENSIVE DATABASE COMPARISON RESULTS")
        print(f"{'=' * 70}")
        
        print(f"\n🎯 Performance Comparison:")
        print(f"{'Database':<12} {'Insert':<10} {'Retrieve':<10} {'Search':<10} {'Strategy':<15} {'Results':<8}")
        print("-" * 70)
        ...
        print(f"   • Fastest Insert: {fastest_insert['db_name']} ({fastest_insert['insert_time']:.3f}s)")
        
        # Retrieval performance
        fastest_retrieval = min(all_results, key=lambda x: x['retrieval_time'])
        print(f"   • Fastest Retrieval: {fastest_retrieval['db_name']} ({fastest_retrieval['retrieval_time']:.3f}s)")
        
        # Search performance
        fastest_search = min(all_results, key=lambda x: x['search_time'])
        print(f"   • Fastest Search: {fastest_search['db_name']} ({fastest_search['search_time']:.3f}s)")
        
        # Most search results
        most_results = max(all_results, key=lambda x: x['search_results'])
        print(f"   • Best Search Results: {most_results['db_name']} ({most_results['search_results']} results)")
        ...
        # Recommendations
        print(f"\n💡 Recommendations:")
        print(f"   • SQLite: Best for single-user applications, embedded systems, development")
        print(f"   • MySQL: Best for web applications, high concurrency, proven reliability")
        print(f"   • PostgreSQL: Best for complex queries, advanced features, data integrity")
        print(f"   • All databases: Full cross-compatibility maintained with identical APIs")
