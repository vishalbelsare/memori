...
def run_mysql_test_scenario(test_name, conscious_ingest, auto_ingest, test_inputs):
    ...
    print(f"Database: MySQL with FULLTEXT search")
    print(f"{'='*60}\n")
    ...
    print(f"✅ Memori enabled with MySQL backend")
    ...
    print(f"📊 Initial stats: {stats_initial['chat_history_count']} chats, {stats_initial['short_term_count']} short-term, {stats_initial['long_term_count']} long-term")
    ...
    chat_growth = stats_final['chat_history_count'] - stats_initial['chat_history_count']
    short_term_growth = stats_final['short_term_count'] - stats_initial['short_term_count']
    ...
    if conscious_ingest:
        print(f"     • Should create detailed memory processing and analysis")
        print(f"     • Should extract entities, keywords, and relationships")
        print(f"     • Should create both short and long-term memories")
    else:
        print(f"     • Should create basic chat history only")
        print(f"     • Minimal memory processing")
    
    print(f"   - Expected behavior with auto_ingest={auto_ingest}:")
    if auto_ingest:
        print(f"     • Should automatically process conversations")
        print(f"     • Should create memories without manual triggers")
    else:
        print(f"     • Should only store basic chat history")
        print(f"     • No automatic memory creation")
    ...
    if chat_growth > 0:
        ...
        for term in search_terms:
            search_results = memory.db_manager.search_memories(term, namespace=test_namespace)
            if search_results:
        ...
        if total_search_results == 0:
            print(f"   ⚠️  No search results found - testing fallback search...")
            # Try a broader search
            all_results = memory.db_manager.search_memories("", namespace=test_namespace, limit=5)
        ...
        print(f"   - LiteLLM callback configuration problems")
        print(f"   - Memory processing disabled")
...
def main():
    ...
    print(f"🧪 Testing {len(test_scenarios)} MySQL configurations with {len(test_inputs)} inputs each\n")
    
    # Clear previous test data
    try:
        from memori import Memori
        cleanup_memory = Memori("mysql+mysqlconnector://root:@127.0.0.1:3306/memori_test")
    ...
    print(f"\n{'='*60}")
    print(f"🏁 MYSQL TEST SUITE RESULTS")
    print(f"{'='*60}")
    print(f"   - Completed tests: {successful_tests}/{len(test_scenarios)}")
    print(f"   - Database: MySQL v{mysql_msg.split()[1] if 'MySQL' in mysql_msg else 'Unknown'}")
    print(f"   - LiteLLM integration: ✅")
    print(f"   - Cross-database compatibility: ✅")
    
    if test_results:
        print(f"\n📊 DETAILED CONFIGURATION ANALYSIS:")
        print(f"{'Test Name':<35} {'API Calls':<10} {'Chat':<6} {'STM':<4} {'LTM':<4} {'Status':<20}")
        print(f"{'-'*80}")
        ...
        for result in test_results:
            ...
            print(f"{result['test_name']:<35} {api_ratio:<10} {result['chat_growth']:<6} "
        ...
        print(f"{'-'*80}")
        print(f"{'TOTALS':<35} {total_api_calls:<10} {total_chat_records:<6} {'N/A':<4} {'N/A':<4} {'Summary':<20}")
        ...
        print(f"   • MySQL FULLTEXT search: Fully operational")
        print(f"   • Cross-database compatibility: Maintained")
        
        # Analyze configuration patterns
        conscious_true_tests = [r for r in test_results if r['conscious_ingest'] == True]
        auto_true_tests = [r for r in test_results if r['auto_ingest'] == True]
        
        if conscious_true_tests:
            avg_memories_conscious = sum(r['short_term_growth'] + r['long_term_growth'] 
                                       for r in conscious_true_tests) / len(conscious_true_tests)
            print(f"   • Average memories with conscious_ingest=True: {avg_memories_conscious:.1f}")
        
        if auto_true_tests:
            avg_chats_auto = sum(r['chat_growth'] for r in auto_true_tests) / len(auto_true_tests)
            print(f"   • Average chat records with auto_ingest=True: {avg_chats_auto:.1f}")
    
    if successful_tests == len(test_scenarios):
        print(f"\n🎉 ALL MYSQL TESTS COMPLETED SUCCESSFULLY!")
        print(f"✅ MySQL backend is fully compatible with LiteLLM integration!")
        print(f"🚀 Ready for production deployment with MySQL database!")
        return 0
    else:
        failed_count = len(test_scenarios) - successful_tests
        print(f"\n⚠️  {failed_count} test(s) failed or incomplete")
        print(f"💡 Check API keys, MySQL connection, and configuration settings")
        return 1
