...
def showcase_memori_v1():
    ...
    personal_assistant.update_user_context(
        ...
        relevant_skills=[
            ...
            "Web Development", "Financial Planning"
        ],
        user_preferences=[
    ...
    ml_memories = personal_assistant.retrieve_context("machine learning gradient descent", limit=3)
    print(f"📊 Found {len(ml_memories)} machine learning memories:")
    for i, memory in enumerate(ml_memories, 1):
        print(f"   {i}. [{memory.get('category_primary', 'unknown')}] {memory.get('summary', 'No summary')[:80]}...")
    ...
    for stat in integration_stats:
        print(f"   🔌 {stat.get('integration', 'unknown')}: {stat.get('active_instances', 0)} instances")
    ...
    try:
        ...
        print(f"   📊 Updated total conversations: {new_stats.get('chat_history_count', 0)}")
    ...
    print(f"   📊 Average Memory Importance: {final_stats.get('average_importance', 0):.2f}")
    ...
    print(f"\n💡 Next Steps:")
    print(f"   • Explore the database: sqlite3 memori_v1_showcase.db")
    print(f"   • Check memory_search_fts table for full-text search")
    print(f"   • Examine memory_entities table for entity indexing")
    print(f"   • Review processed_data JSON for structured memories")
