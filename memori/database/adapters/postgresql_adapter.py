...
class PostgreSQLSearchAdapter(BaseSearchAdapter):
    ...
    def __init__(self, connector: BaseDatabaseConnector):
        ...
        self.query_executor = DatabaseQueryExecutor(connector, DatabaseDialect.POSTGRESQL)
    ...
    def execute_fulltext_search(
        ...
        try:
            ...
            if not self._check_fts_available():
                logger.debug("PostgreSQL FTS not available, falling back to LIKE search")
        ...
        try:
            with self.connector.get_connection() as conn:
                ...
                try:
                    ...
                    logger.debug(f"Concurrent reindex failed, trying regular reindex: {e}")
                    try:
    ...
    def create_materialized_views(self):
        ...
        try:
            with self.connector.get_connection() as conn:
                ...
                conn.commit()
                logger.info("Created PostgreSQL materialized view for search optimization")
    ...
    def refresh_materialized_views(self):
        ...
        try:
            with self.connector.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("REFRESH MATERIALIZED VIEW CONCURRENTLY memory_search_mv")
                conn.commit()
