...
from ..connectors.base_connector import BaseSearchAdapter, BaseDatabaseConnector
from ...utils.query_builder import DatabaseQueryExecutor, DatabaseDialect
...
class MySQLSearchAdapter(BaseSearchAdapter):
    ...
    def execute_fulltext_search(
        ...
        try:
            ...
            if not self._check_fts_available():
                logger.debug("MySQL FULLTEXT not available, falling back to LIKE search")
    ...
    def translate_search_query(self, query: str) -> str:
        ...
        words = [word.strip() for word in sanitized.split() if word.strip() and len(word) >= 3]  # MySQL ft_min_word_len
    ...
    def _check_fts_available(self) -> bool:
        ...
        try:
            with self.connector.get_connection() as conn:
                ...
                version_ok = (version_parts[0] > 5 or (version_parts[0] == 5 and version_parts[1] >= 6))
        ...
        try:
            with self.connector.get_connection() as conn:
                ...
                for setting, value in recommended_settings.items():
                    if setting in current_settings and current_settings[setting] != value:
                        logger.info(f"Consider setting {setting} = {value} (current: {current_settings.get(setting)})")
