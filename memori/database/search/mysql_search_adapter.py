...
class MySQLSearchAdapter(BaseSearchAdapter):
    ...
    def _check_fulltext_support(self) -> bool:
        ...
        try:
            ...
            if index_count >= 2:  # Expect FULLTEXT indexes on both tables
                ...
                logger.warning("MySQL FULLTEXT indexes not found, will use LIKE fallback")
                return False
    ...
    def execute_fulltext_search(
        ...
        if self.fulltext_available and query and query.strip():
            try:
                return self._execute_mysql_fulltext_search(query, namespace, category_filter, limit)
            except Exception as e:
                logger.debug(f"MySQL FULLTEXT search failed: {e}, falling back to LIKE search")
