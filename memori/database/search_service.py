...
class SearchService:
    ...
    def search_memories(
        ...
        if not query or not query.strip():
            return self._get_recent_memories(namespace, category_filter, limit, memory_types)
        ...
        try:
            ...
            if self.database_type == 'sqlite':
                results = self._search_sqlite_fts(query, namespace, category_filter, limit, search_short_term, search_long_term)
            elif self.database_type == 'mysql':
                results = self._search_mysql_fulltext(query, namespace, category_filter, limit, search_short_term, search_long_term)
            elif self.database_type == 'postgresql':
                results = self._search_postgresql_fts(query, namespace, category_filter, limit, search_short_term, search_long_term)
            
            # If no results or full-text search failed, fall back to LIKE search
            if not results:
                results = self._search_like_fallback(query, namespace, category_filter, limit, search_short_term, search_long_term)
    ...
    def _search_mysql_fulltext(
        ...
        try:
            ...
            if search_short_term:
                ...
                # Add category filter
                if category_filter:
                    short_query = short_query.filter(ShortTermMemory.category_primary.in_(category_filter))
                
                # Add relevance score
                short_results = self.session.execute(
                    short_query.statement.add_columns(
                        text("MATCH(searchable_content, summary) AGAINST(:query IN NATURAL LANGUAGE MODE) as search_score").params(query=query),
            ...
            if search_long_term:
                ...
                # Add category filter
                if category_filter:
                    long_query = long_query.filter(LongTermMemory.category_primary.in_(category_filter))
    ...
    def _search_postgresql_fts(
        ...
        try:
            ...
            if search_short_term:
                ...
                # Add tsvector search
                ts_query = text("search_vector @@ to_tsquery('english', :query)").params(query=tsquery_text)
                ...
                short_results = self.session.execute(
                    short_query.statement.add_columns(
                        text("ts_rank(search_vector, to_tsquery('english', :query)) as search_score").params(query=tsquery_text),
                        text("'short_term' as memory_type"),
    ...
    def _search_like_fallback(
        ...
        if search_short_term:
            ...
            short_results = short_query.order_by(
        ...
        if search_long_term:
            ...
            long_results = long_query.order_by(
    ...
    def _get_recent_memories(
        ...
        if search_short_term:
            ...
            short_results = short_query.order_by(desc(ShortTermMemory.created_at)).limit(limit // 2).all()
        ...
        if search_long_term:
            ...
            long_results = long_query.order_by(desc(LongTermMemory.created_at)).limit(limit // 2).all()
    ...
    def _rank_and_limit_results(self, results: List[Dict[str, Any]], limit: int) -> List[Dict[str, Any]]:
