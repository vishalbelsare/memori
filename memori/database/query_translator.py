...
class QueryParameterTranslator:
    ...
    def _translate_boolean(self, value: bool) -> Union[bool, int]:
        ...
        if self.database_type == 'postgresql':
            ...
            logger.warning(f"Unknown database type {self.database_type}, using default boolean handling")
            return value
    
    def _is_likely_boolean_context(self, value: int, parameter_name: str = None) -> bool:
        """
        ...
        if parameter_name:
            boolean_patterns = [
                'active', 'enabled', 'disabled', 'processed', 'eligible', 'is_', 
                'has_', 'can_', 'should_', 'allow_', 'visible', 'hidden',
                'conscious_processed', 'processed_for_duplicates', 'promotion_eligible',
                'is_user_context', 'is_preference', 'is_skill_knowledge', 
                'is_current_project', 'is_permanent_context'
            ]
    ...
    def translate_query_with_parameters(self, query: str, parameters: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
        """
...
def translate_query_params(parameters: Dict[str, Any], database_type: str) -> Dict[str, Any]:
    """
