...
class MySQLConnector(BaseDatabaseConnector):
    ...
    def _parse_connection_string(self, connection_string: str) -> Dict[str, Any]:
        ...
        if connection_string.startswith("mysql://"):
            ...
            raise DatabaseError(f"Invalid MySQL connection configuration: {connection_string}")
    ...
    def get_connection(self):
        ...
        try:
            ...
            cursor.execute("SET SESSION ft_min_word_len = 3")  # FULLTEXT minimum word length
            cursor.close()
    ...
    def initialize_schema(self, schema_sql: Optional[str] = None):
        ...
        try:
            if not schema_sql:
                ...
                from ..schema_generators.mysql_schema_generator import MySQLSchemaGenerator
        ...
        try:
            with self.get_connection() as conn:
                ...
                innodb_available = any(
                    engine['Engine'] == 'InnoDB' and engine['Support'] in ('YES', 'DEFAULT')
                    for engine in engines
                )
                
                return (version_parts[0] > 5 or 
                       (version_parts[0] == 5 and version_parts[1] >= 6)) and innodb_available
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                info['character_sets'] = {var['Variable_name']: var['Value'] for var in charset_vars}
                
                # FULLTEXT configuration
                cursor.execute("SHOW VARIABLES LIKE 'ft_%'")
                fulltext_vars = cursor.fetchall()
                info['fulltext_config'] = {var['Variable_name']: var['Value'] for var in fulltext_vars}
