...
class ConsciouscAgent:
    ...
    async def run_conscious_ingest(
        ...
        try:
            ...
            memory_ids = [row[0] for row in conscious_memories]  # memory_id is first column
            await self._mark_memories_processed(db_manager, memory_ids, namespace)
            
            self.context_initialized = True
            logger.info(f"ConsciouscAgent: Copied {copied_count} conscious-info memories to short-term memory")
    ...
    async def check_for_context_updates(
        ...
        try:
            ...
            logger.info(f"ConsciouscAgent: Copied {copied_count} new conscious-info memories to short-term memory")
            return copied_count > 0
    ...
    async def _copy_memory_to_short_term(
        ...
        try:
            memory_id, processed_data, summary, searchable_content, importance_score, _ = memory_row
            ...
            logger.debug(f"ConsciouscAgent: Copied memory {memory_id} to short-term as {short_term_id}")
            return True
            
        except Exception as e:
            logger.error(f"ConsciouscAgent: Failed to copy memory {memory_row[0]} to short-term: {e}")
            return False
