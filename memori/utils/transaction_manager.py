...
class TransactionManager:
        ...
        try:
            ...
            try:
                ...
                if timeout and (time.time() - start_time) > timeout:
                    raise DatabaseError(f"Transaction {transaction_id} timed out after {timeout}s")
            ...
            if self.current_transaction:
                ...
                logger.debug(f"Transaction {transaction_id} completed in {execution_time:.3f}s")
                self.current_transaction = None
                ...
                for i, operation in enumerate(operations):
                    try:
                        ...
                        if operation.expected_rows is not None:
                            if hasattr(result, '__len__') and len(result) != operation.expected_rows:
                                raise DatabaseError(f"Operation {i} affected {len(result)} rows, expected {operation.expected_rows}")
                        
                        completed_ops += 1
                        
                    except Exception as e:
                        logger.error(f"Operation {i} failed: {e}")
                        raise DatabaseError(f"Operation {i} ({operation.operation_type}) failed: {e}")
    ...
    def execute_with_retry(
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    logger.warning(f"Operation failed (attempt {attempt + 1}/{retries + 1}), retrying in {delay}s: {e}")
                    time.sleep(delay)
                    delay *= 2  # Exponential backoff
                else:
                    logger.error(f"Operation failed after {retries + 1} attempts: {e}")
        
        raise DatabaseError(f"Operation failed after {retries + 1} attempts: {last_error}")
        ...
        try:
            if hasattr(conn, 'set_isolation_level'):
                ...
                if isolation_level != IsolationLevel.READ_COMMITTED:  # SQLite default
                    cursor.execute(f"PRAGMA read_uncommitted = {'ON' if isolation_level == IsolationLevel.READ_UNCOMMITTED else 'OFF'}")
    ...
    def _is_retryable_error(self, error: Exception) -> bool:
        ...
        retryable_patterns = [
            ...
            'busy', 'lock', 'deadlock', 'serialization'
        ]
        
        # Non-retryable error patterns
        non_retryable_patterns = [
            'constraint', 'unique', 'foreign key', 'not null',
            'syntax error', 'permission', 'access denied'
        ]
...
class TransactionContext:
    ...
    def execute_many(self, query: str, params_list: List[List[Any]]) -> int:
        ...
        try:
            ...
            logger.error(f"Batch execution failed in transaction {self.transaction_id}: {e}")
    ...
    def execute_script(self, script: str):
        ...
        try:
            ...
            logger.error(f"Script execution failed in transaction {self.transaction_id}: {e}")
...
def bulk_insert_transaction(
    ...
    for i in range(0, len(data), batch_size):
        ...
        for row in batch:
            ...
            placeholders = ','.join(['?' if connector.database_type.value == 'sqlite' else '%s'] * len(columns))
