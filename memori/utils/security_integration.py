...
from .query_builder import QueryBuilder, DatabaseQueryExecutor, DatabaseDialect
from .transaction_manager import TransactionManager, TransactionOperation
from .security_audit import DatabaseSecurityAuditor, get_security_auditor
...
class SecureMemoriDatabase:
        ...
        try:
            ...
            if use_fts:
                try:
                    ...
                    logger.debug(f"FTS query building failed, falling back to LIKE: {e}")
                    use_fts = False
            ...
            if not is_safe:
                critical_findings = [f for f in security_findings if f.severity.value in ['critical', 'high']]
    ...
    def secure_update(
        ...
        try:
            # Phase 1: Input validation
            validated_data = {
                k: InputValidator.validate_text_content(str(v)) if isinstance(v, str) else v
...
def validate_memori_security_config() -> Dict[str, Any]:
    ...
    try:
        ...
        test_query = InputValidator.validate_and_sanitize_query("test query")
        ...
        findings = auditor.audit_query("SELECT * FROM test", None, "validation")
        validation_results['security_auditor'] = True
        
        # Check database adapters
        try:
            from ..database.adapters import SQLiteSearchAdapter, PostgreSQLSearchAdapter, MySQLSearchAdapter
