...
class InputValidator:
    ...
    def validate_and_sanitize_query(cls, query: str, max_length: int = 10000) -> str:
        ...
        for pattern in cls.SQL_INJECTION_PATTERNS:
            if re.search(pattern, query_lower, re.IGNORECASE):
                ...
                raise ValidationError("Invalid query: contains potentially dangerous content")
    ...
    def validate_namespace(cls, namespace: str) -> str:
        ...
        if not re.match(r'^[a-zA-Z0-9_\-]+$', sanitized_namespace):
            raise ValidationError("Namespace contains invalid characters (only alphanumeric, underscore, hyphen allowed)")
    ...
    @classmethod
    def validate_category_filter(cls, category_filter: Optional[List[str]]) -> List[str]:
    ...
    def validate_json_field(cls, json_data: Any, field_name: str = "data") -> str:
        ...
        try:
            if isinstance(json_data, str):
                ...
                clean_json = json.dumps(parsed_data, ensure_ascii=True, separators=(',', ':'))
            else:
                # Serialize Python object to JSON
                clean_json = json.dumps(json_data, ensure_ascii=True, separators=(',', ':'))
    ...
    @classmethod
    def validate_text_content(cls, content: str, field_name: str = "content", max_length: int = 100000) -> str:
    ...
    @classmethod
    def validate_score(cls, score: Union[float, int, str], field_name: str = "score") -> float:
    ...
    def sanitize_sql_identifier(cls, identifier: str) -> str:
        ...
        reserved_words = {
            ...
            'FUNCTION', 'EXEC', 'EXECUTE', 'UNION', 'WHERE', 'FROM', 'JOIN'
        }
        
        if sanitized.upper() in reserved_words:
            raise ValidationError(f"Cannot use reserved word as identifier: {sanitized}")
...
class DatabaseInputValidator:
    ...
    @classmethod
    def validate_insert_params(cls, table: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in params.items():
            ...
            if key.endswith('_id'):
                if value is not None:
                    sanitized_params[sanitized_key] = InputValidator.validate_memory_id(str(value))
                else:
                    sanitized_params[sanitized_key] = None
            elif key == 'namespace':
                sanitized_params[sanitized_key] = InputValidator.validate_namespace(str(value))
            elif key.endswith('_score'):
                sanitized_params[sanitized_key] = InputValidator.validate_score(value, key)
            elif key.endswith('_at') or key == 'timestamp':
                sanitized_params[sanitized_key] = InputValidator.validate_timestamp(value)
            elif key.endswith('_json') or key == 'metadata':
                sanitized_params[sanitized_key] = InputValidator.validate_json_field(value, key)
            elif isinstance(value, bool) or key.startswith('is_'):
                sanitized_params[sanitized_key] = InputValidator.validate_boolean_field(value, key)
