...
class DatabaseSecurityAuditor:
    ...
    def _audit_sql_injection(
        ...
        if any(char in query for char in ['+', '||']) and 'VALUES' in query.upper():
            findings.append(SecurityFinding(
                ...
                evidence=f"Query contains string concatenation operators"
            ))
    ...
    def _audit_privilege_operations(
        ...
        system_functions = [
            'SYSTEM', 'SHELL', 'EXEC', 'EXECUTE', 'XP_CMDSHELL',
    ...
    def generate_audit_report(self) -> SecurityAuditReport:
        ...
        critical_count = sum(1 for f in self.findings if f.severity == SecurityLevel.CRITICAL)
        high_count = sum(1 for f in self.findings if f.severity == SecurityLevel.HIGH)
        medium_count = sum(1 for f in self.findings if f.severity == SecurityLevel.MEDIUM)
...
class SecureQueryBuilder:
    ...
    def build_safe_select(
        ...
        validated_columns = [InputValidator.sanitize_sql_identifier(col) for col in columns]
        ...
        # Audit the query
        is_safe, findings = self.auditor.validate_query_safety(query, params, "build_safe_select")
        if not is_safe:
    ...
    def build_safe_insert(
        ...
        # Audit the query
        is_safe, findings = self.auditor.validate_query_safety(query, values, "build_safe_insert")
        if not is_safe:
            raise SecurityError(f"Generated query failed security audit: {findings}")
...
def audit_query(query: str, params: Optional[List[Any]] = None, context: Optional[str] = None) -> List[SecurityFinding]:
    """Convenience function to audit a single query"""
    return _global_auditor.audit_query(query, params, context)

def validate_query_safety(query: str, params: Optional[List[Any]] = None, context: Optional[str] = None) -> bool:
