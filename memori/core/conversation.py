...
class ConversationSession:
    ...
    def get_history_messages(self, limit: int = 10) -> List[Dict[str, str]]:
        ...
        recent_messages = user_assistant_messages[-limit:] if limit > 0 else user_assistant_messages
...
class ConversationManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"ConversationManager initialized: max_sessions={max_sessions}, "
    ...
    def add_user_message(self, session_id: str, content: str, metadata: Dict[str, Any] = None):
        ...
        if len(session.messages) > self.max_history_per_session:
            ...
            recent_messages = other_messages[-(self.max_history_per_session - len(system_messages)):]
    ...
    def inject_context_with_history(self, 
                                   session_id: str,
                                   messages: List[Dict[str, str]],
                                   memori_instance,
                                   mode: str = "conscious") -> List[Dict[str, str]]:
        """
        ...
        try:
            ...
            if len(history_messages) > 1:  # More than just current message
                ...
                if previous_messages:
                    ...
                    logger.debug(f"Added {len(previous_messages)} history messages for session {session_id}")
            ...
            logger.debug(f"Enhanced messages for session {session_id}: context={'yes' if context_prompt else 'no'}, "
                        f"history={'yes' if len(history_messages) > 1 else 'no'}")
            
            return enhanced_messages
            
        except Exception as e:
            logger.error(f"Failed to inject context with history for session {session_id}: {e}")
            return messages
    
    def record_response(self, session_id: str, response: str, metadata: Dict[str, Any] = None):
