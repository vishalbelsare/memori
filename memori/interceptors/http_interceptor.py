...
class HTTPInterceptor(ConversationInterceptor):
    ...
    def _install_httpx_middleware(self):
        ...
        class MemoriHTTPXTransport(httpx.HTTPTransport):
            ...
            def handle_request(self, request):
                ...
                if (
                    self.memori_interceptor.memori.is_enabled
                    and self.memori_interceptor._is_llm_endpoint(str(request.url))
                ):
    ...
    def _install_requests_middleware(self):
        ...
        class MemoriHTTPAdapter(HTTPAdapter):
            ...
            def send(self, request, **kwargs):
                ...
                if (
                    self.memori_interceptor.memori.is_enabled
                    and self.memori_interceptor._is_llm_endpoint(request.url)
                ):
    ...
    def _parse_and_record_http_request(
        self, method: str, url: str, request_data: Dict[str, Any], response: Any
    ):
    ...
    def _extract_conversation_from_http(
        self, method: str, url: str, request_data: Dict[str, Any], response: Any
    ) -> tuple[str, str, str]:
        """Extract conversation data from HTTP request/response"""
        user_input = ""
        ...
        try:
            # Parse request data
            request_content = (
                request_data.get("content") or request_data.get("data") or b""
            )
            ...
            if "api.openai.com" in url:
                user_input, ai_output, model = self._parse_openai_http(
                    request_json, response_json
                )
            elif "api.anthropic.com" in url:
                user_input, ai_output, model = self._parse_anthropic_http(
                    request_json, response_json
                )
            else:
                # Generic parsing
                user_input, ai_output, model = self._parse_generic_http(
                    request_json, response_json
                )
