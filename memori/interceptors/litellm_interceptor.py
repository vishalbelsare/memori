...
class LiteLLMNativeInterceptor(ConversationInterceptor):
    ...
    def enable(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                if hasattr(litellm, "input_callback") and not hasattr(
                    self, "_input_callback_registered"
                ):
                    ...
                    def enhanced_completion(*args, **kwargs):
                        ...
                        if self.memori.is_enabled:
                            if self.memori.auto_ingest:
                                kwargs = self.memori._inject_litellm_context(
                                    kwargs, mode="auto"
                                )
                    ...
                    logger.debug(
                        "LiteLLM: Using function wrapping (input callbacks not available)"
                    )
                else:
        ...
        with self._lock:
            ...
            try:
                ...
                if (
                    ...
                    and self._litellm_success_callback in success_callback
                ):
                    success_callback.remove(self._litellm_success_callback)
                    self._callback_registered = False

                # Remove input callback if registered
                if hasattr(self, "_input_callback_registered") and hasattr(
                ...
                if self._original_completion and hasattr(
                    litellm.completion, "_memori_enhanced"
                ):
    ...
    def _litellm_success_callback(
        ...
        try:
            self.memori._process_litellm_response(
                kwargs, response, start_time, end_time
            )
            self._record_success()
        except Exception as e:
