...
class InterceptorManager:
    ...
    def enable(self, methods: List[str] = None) -> Dict[str, bool]:
        ...
        with self._lock:
            ...
            method_mapping = {
                ...
                'all': [LiteLLMNativeInterceptor, OpenAIClientInterceptor, AnthropicClientInterceptor, HTTPInterceptor]
            }
    ...
    def disable(self) -> Dict[str, bool]:
        ...
        with self._lock:
            ...
            for interceptor in self._enabled_interceptors[:]:  # Copy list to avoid modification during iteration
    ...
    def get_interceptor_status(self) -> Dict[str, Any]:
        ...
        available_names = [i.name for i in self.interceptors if self._check_interceptor_availability(i)]
    ...
    def _check_interceptor_availability(self, interceptor: ConversationInterceptor) -> bool:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        for interceptor in self.interceptors:
            if interceptor.is_enabled:
                ...
                if not self._check_interceptor_availability(interceptor):
                    health['issues'].append(f"{interceptor.name} is enabled but dependencies are not available")
        ...
        if len(self._enabled_interceptors) == 0:
            ...
            health['recommendations'].append("Enable interceptors using enable() method")
            health['overall_status'] = 'unhealthy'
        
        # Check for optimal configuration
        litellm_enabled = any(isinstance(i, LiteLLMNativeInterceptor) and i.is_enabled for i in self.interceptors)
        if not litellm_enabled:
            health['recommendations'].append("Consider enabling LiteLLM interceptor for best performance")
