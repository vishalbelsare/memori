    ...
    def enable(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                openai.OpenAI = self._create_wrapped_openai_client(openai._memori_original_OpenAI)
                openai.AsyncOpenAI = self._create_wrapped_async_openai_client(openai._memori_original_AsyncOpenAI)
                ...
                try:
                    ...
                    openai.AzureOpenAI = self._create_wrapped_azure_client(openai._memori_original_AzureOpenAI)
                    openai.AsyncAzureOpenAI = self._create_wrapped_async_azure_client(openai._memori_original_AsyncAzureOpenAI)
                except ImportError:
        ...
        class MemoriOpenAIClient(original_class):
            ...
            def _memori(self):
                ...
                if memori_instance is None:
                    logger.warning(f"{self.__class__.__name__}: Memori instance garbage collected, disabling interception")
                    return None
                ...
                class WrappedChatCompletions:
                    ...
                    def completions(self):
                        if not hasattr(self, '_memori_completions'):
                            self._memori_completions = self._wrap_completions_create(self._original.completions)
                            ...
                            def create(self, **kwargs):
                                ...
                                try:
                                    ...
                                    if not memori_instance:
                                        return self._original.create(**kwargs)  # Gracefully fallback
                                    
                                    # Inject context if enabled (auto_ingest OR conscious_ingest)
                                    if memori_instance.is_enabled and (memori_instance.auto_ingest or memori_instance.conscious_ingest):
                                        kwargs = memori_instance._inject_openai_context(kwargs)
                                    ...
                                    if memori_instance.is_enabled:
                                        memori_instance._record_openai_conversation(kwargs, response)
        ...
        class MemoriAsyncOpenAIClient(original_class):
            ...
            def _wrap_async_chat_completions(self, original_chat):
                ...
                class WrappedAsyncChatCompletions:
                    ...
                    def completions(self):
                        if not hasattr(self, '_memori_completions'):
                            self._memori_completions = self._wrap_async_completions_create(self._original.completions)
                        return self._memori_completions
                    ...
                    def _wrap_async_completions_create(self, original_completions):
                        ...
                        class WrappedAsyncCompletions:
                            ...
                            async def create(self, **kwargs):
                                # Inject context if enabled (auto_ingest OR conscious_ingest)
                                if memori.is_enabled and (memori.auto_ingest or memori.conscious_ingest):
