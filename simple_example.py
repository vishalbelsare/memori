...
import os
from memoriai import Memori, MemoryCategoryType
...
def main():
    ...
    for stat in integration_stats:
        print(f"  📡 {stat.get('integration')}: {stat.get('active_instances', 0)} instances")
    ...
    print(f"\n📈 Updated Statistics:")
    print(f"  📊 Total memories: {updated_stats.get('short_term_count', 0) + updated_stats.get('long_term_count', 0)}")
