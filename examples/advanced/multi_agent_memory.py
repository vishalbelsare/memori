...
def main():
    ...
    comm_info = communication_search("professional communication preferences", max_results=1)
    ...
    collaboration_task = "Plan a quantum computing research project with proper communication strategy"
    
    # Each agent contributes based on their expertise
    research_contribution = research_agent.retrieve_context("quantum computing", limit=1)
    planning_contribution = planning_agent.retrieve_context("project planning", limit=1)
    communication_contribution = communication_agent.retrieve_context("communication strategy", limit=1)
    ...
    for i, memory in enumerate(collaborative_memories, 1):
        ...
        agent_context = "Research" if i == 1 else "Planning" if i == 2 else "Communication"
    ...
    for agent_name, agent in agents.items():
        collab_chat_id = agent.record_conversation(
            ...
            metadata={"collaboration": True, "multi_agent": True, "task": "quantum_research_planning"}
        )
    ...
    for agent_name, agent in agents.items():
        ...
        print(f"  🧠 Memories: {stats.get('short_term_count', 0) + stats.get('long_term_count', 0)}")
        print(f"  🏷️  Entities: {stats.get('total_entities', 0)}")
        
        categories = stats.get('memories_by_category', {})
        if categories:
            print(f"  📊 Top category: {max(categories.items(), key=lambda x: x[1])[0] if categories else 'None'}")
        print()
    ...
    research_context = research_agent.retrieve_context("quantum computing sustainable energy", limit=2)
    decision_inputs["research"] = f"Based on research data: {len(research_context)} relevant findings available"
    
    # Planning agent input
    planning_context = planning_agent.retrieve_context("project planning timeline", limit=1)
    decision_inputs["planning"] = f"Based on planning expertise: {len(planning_context)} planning considerations"
    
    # Communication agent input
    communication_context = communication_agent.retrieve_context("communication preferences", limit=1)
    decision_inputs["communication"] = f"Based on communication needs: {len(communication_context)} stakeholder factors"
    ...
    total_conversations = sum(agent.get_memory_stats().get('chat_history_count', 0) for agent in agents.values())
    total_memories = sum(
        agent.get_memory_stats().get('short_term_count', 0) + 
        agent.get_memory_stats().get('long_term_count', 0) 
        for agent in agents.values()
    )
    total_entities = sum(agent.get_memory_stats().get('total_entities', 0) for agent in agents.values())
    
    print(f"Multi-Agent System Metrics:")
    print(f"  🤖 Active Agents: {len(agents)}")
    print(f"  💬 Total Conversations: {total_conversations}")
    print(f"  🧠 Total Memories: {total_memories}")
    print(f"  🏷️  Total Entities: {total_entities}")
    print(f"  🔄 Collaboration Events: 4")  # Based on examples above
    ...
    print("💾 Check agent databases: research_agent.db, planning_agent.db, communication_agent.db")
