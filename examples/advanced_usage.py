...
import os
import json
from memoriai import Memori, create_memory_search_tool, MemoryCategoryType, RetentionType
...
def main():
    ...
    learning_search_result = learning_search_tool("architecture patterns", max_results=2)
    learning_data = json.loads(learning_search_result)
    print(f"   📚 Learning search found {learning_data['found']} architecture-related memories")
    ...
    print(f"  📚 Learning focus: {learning_stats.get('chat_history_count', 0)} conversations")
    ...
    if total_entities > 0:
        ...
        print(f"  📊 Entity distribution: Work {work_ratio:.1f}% | Learning {learning_ratio:.1f}%")
