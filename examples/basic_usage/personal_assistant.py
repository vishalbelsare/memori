...
def main():
    ...
    morning_context = assistant_memory.retrieve_context("morning coffee preference", limit=1)
    context_summary = ""
    if morning_context:
    ...
    context_aware_chat_id = assistant_memory.record_conversation(
        ...
        ai_output=f"Based on your preferences (you prefer coffee over tea in the mornings), I'd suggest having coffee with your breakfast. For food, consider something protein-rich like eggs or yogurt to start your day well!",
    ...
    print(f"📝 Used context: {context_summary[:50] if context_summary else 'No specific context'}...")
    
    # Personal assistant statistics
    print("\n📊 Personal Assistant Statistics...")
    
    stats = assistant_memory.get_memory_stats()
    print(f"Assistant Memory Stats:")
    ...
    print(f"📄 Personal data export ready")
