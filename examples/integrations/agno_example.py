...
def main():
    ...
    for i, memory in enumerate(goal_memories, 1):
        ...
        print(
            f"  {i}. [{category.upper()}] {summary[:70]}... (importance: {importance:.2f})"
        )

    # Example 3: Navigation strategy memories
    print("\n🗺️  Example 3: Navigation strategy memory analysis...")

    navigation_memories = agno_memory.retrieve_context(
        "decision tree navigation", limit=2
    )
    ...
    print(
        f"  🎯 Total Goal-oriented Conversations: {stats.get('chat_history_count', 0)}"
    )
    print(
        f"  🧠 Stored Memories: {stats.get('short_term_count', 0) + stats.get('long_term_count', 0)}"
    )
    ...
    decision_memories = agno_memory.retrieve_context(
        "exploration exploitation strategies", limit=3
    )
    ...
    agno_insights = {
        ...
        "goal_oriented_sessions": len(
            [c for c in recent_conversations if "agno" in str(c.get("metadata", {}))]
        ),
        "optimization_strategies": len(
            [
                c
                for c in recent_conversations
                if "optimization" in str(c.get("metadata", {}))
            ]
        ),
        "decision_points": len(
            [
                c
                for c in recent_conversations
                if "decision" in str(c.get("metadata", {}))
            ]
        ),
