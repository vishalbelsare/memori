...
def main():
    ...
    try:
        response1 = completion(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "user", "content": "What are the benefits of using LiteLLM for multi-model AI applications?"}
            ]
        )
    ...
    for model in models_to_test:
        try:
            ...
            response = completion(
                model=model,
                messages=[
                    {"role": "user", "content": f"Explain the advantages of {model} in a concise way"}
                ]
            )
    ...
    conversation_messages = [
        {"role": "user", "content": "I'm building a chatbot. What should I consider for memory management?"}
    ]
    ...
    try:
        ...
        conversation_messages.append({
            "role": "user", 
            "content": "How would Memoriai help with this use case?"
        })
        ...
        print(f"📄 Final response preview: {response2.choices[0].message.content[:100]}...")
    ...
    try:
        auto_response = completion(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "user", "content": "What factors should I consider when choosing between different AI models?"}
            ]
        )
    ...
    print(f"\n📊 Final Statistics:")
    print(f"  📈 Total conversations processed: {final_stats.get('chat_history_count', 0)}")
