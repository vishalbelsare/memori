...
def run_postgresql_test_scenario(test_name, conscious_ingest, auto_ingest, test_inputs):
    ...
    init_kwargs = {"database_connect": postgresql_connection, "verbose": True}
    ...
    search_results = memory.db_manager.search_memories(
        "test PostgreSQL", namespace=test_namespace
    )
    test_results["search_results_count"] = len(search_results)

    # Check search strategy used
    if search_results:
        test_results["search_strategy"] = search_results[0].get(
...
def main():
    ...
    print("🗃️  Testing with PostgreSQL backend")
    ...
    try:
        ...
        print(
            "💡 Make sure PostgreSQL is running and setup_postgresql.py has been executed"
        )
        return 1
    ...
    if len(all_results) > 0:
        ...
        print(
            f"{'Scenario':<40} {'Success':<8} {'Failed':<8} {'Chats':<8} {'Search':<8} {'Duration':<10}"
        )
        print("-" * 75)
        ...
        for result in all_results:
            ...
            print(
                f"{result['scenario']:<40} {result['successful_calls']:<8} "
        ...
        print("-" * 75)
        print(
            f"{'TOTALS':<40} {total_success:<8} {total_failed:<8} {total_chats:<8} {'':<8} {total_duration:.1f}s"
        )
        ...
        print("   • PostgreSQL tsvector search working across all scenarios")

        # Database backend confirmation
        print("\n🗄️  Database Backend Confirmation:")
        print("   • All scenarios used PostgreSQL backend successfully")
        print("   • Cross-database compatibility maintained")
        print("   • SQLAlchemy abstraction layer working correctly")
        print("   • PostgreSQL tsvector full-text search operational")
        ...
        if success_rate >= 80:
            ...
            print(
                "⚠️  PostgreSQL + LiteLLM integration is working but may need optimization"
            )
        else:
