...
from ..utils.pydantic_models import EntityType, MemoryCategoryType
...
class EssentialMemory(BaseModel):
    ...
    frequency_score: float = Field(ge=0.0, le=1.0, description="How frequently this is referenced")
    recency_score: float = Field(ge=0.0, le=1.0, description="How recent this information is")
...
class EssentialMemoriesAnalysis(BaseModel):
    ...
    promoted_count: int = Field(description="Number of memories recommended for promotion")
...
class ConsciouscAgent:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-4o"):
        ...
        if effective_api_key:
            self.client = AsyncOpenAI(api_key=api_key)  # AsyncOpenAI handles None api_key automatically
        else:
            self.client = None
            logger.warning("ConsciouscAgent: No OpenAI API key found. Set OPENAI_API_KEY environment variable or provide api_key parameter.")
...
- Are highly specific to a single context that hasn't been revisited"""
    ...
    async def analyze_memory_patterns(
        ...
        try:
            ...
            if len(memories) < min_memories:
                logger.info(f"ConsciouscAgent: Insufficient memories ({len(memories)}) for analysis")
                return None
            ...
            if analysis:
                ...
                logger.info(f"ConsciouscAgent: Selected {len(analysis.essential_memories)} essential conversations")
    ...
    async def _get_long_term_memories(self, db_manager, namespace: str) -> List[Dict[str, Any]]:
    ...
    async def _perform_memory_selection(self, memory_summaries: List[Dict]) -> Optional[EssentialMemoriesAnalysis]:
...
Limit selection to the top 5-10 most essential conversations."""
    ...
    def _prepare_memory_context(self, memory_summaries: List[Dict]) -> str:
        ...
        for i, memory in enumerate(memory_summaries[:50], 1):  # Limit to 50 most important
    ...
    async def update_short_term_memories(
        ...
        try:
            ...
            logger.info(f"ConsciouscAgent: Copied {updated_count} essential conversations to short-term memory")
    ...
    async def _clear_essential_conversations(self, db_manager, namespace: str):
        ...
        try:
            ...
            logger.error(f"ConsciouscAgent: Failed to clear essential conversations: {e}")
    
    async def _copy_conversation_to_short_term(
        self, 
        db_manager, 
        essential_memory: EssentialMemory,
        namespace: str
    ) -> bool:
        ...
        try:
            ...
            original_memory = await self._get_original_memory(db_manager, essential_memory.memory_id)
            
            if not original_memory:
                logger.warning(f"ConsciouscAgent: Could not find original memory {essential_memory.memory_id}")
                return False
            ...
            try:
                original_processed_data = json.loads(original_memory.get('processed_data', '{}'))
            except json.JSONDecodeError:
            ...
            with db_manager._get_connection() as connection:
                ...
                connection.execute(query, (
                    new_memory_id,
                    original_memory.get('original_chat_id'),  # Preserve original chat_id link
                    ...
                    original_memory.get('searchable_content', essential_memory.summary),
            ...
            logger.error(f"ConsciouscAgent: Failed to copy conversation to short-term: {e}")
            return False
    ...
    async def run_background_analysis(self, db_manager, namespace: str = "default"):
        ...
        try:
            ...
            if analysis:
                ...
                logger.info("ConsciouscAgent: Background analysis completed successfully")
            else:
                logger.info("ConsciouscAgent: No analysis performed (insufficient data)")
