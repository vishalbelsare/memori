...
class DataValidator:
    ...
    def validate_namespace(cls, value: str, field_name: str = "namespace") -> str:
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', value):
            raise ValidationError(f"{field_name} can only contain letters, numbers, underscores, and hyphens")
        
        return value
    
    @classmethod
    def validate_importance_score(cls, value: float, field_name: str = "importance score") -> float:
    ...
    @classmethod
    def validate_file_path(cls, value: Union[str, Path], field_name: str = "file path", must_exist: bool = False) -> Path:
        """Validate file path"""
    ...
    @classmethod
    def validate_json_dict(cls, value: Any, field_name: str = "JSON data") -> Dict[str, Any]:
    ...
    @classmethod
    def validate_memory_category(cls, value: str, field_name: str = "memory category") -> str:
    ...
    @classmethod
    def validate_retention_type(cls, value: str, field_name: str = "retention type") -> str:
        ...
        if value not in valid_types:
            raise ValidationError(f"{field_name} must be one of: {', '.join(valid_types)}")
    ...
    def validate_entity_type(cls, value: str, field_name: str = "entity type") -> str:
        ...
        valid_types = ['person', 'technology', 'topic', 'skill', 'project', 'keyword', 'location', 'organization']
        if value not in valid_types:
            raise ValidationError(f"{field_name} must be one of: {', '.join(valid_types)}")
    ...
    @classmethod
    def validate_positive_integer(cls, value: int, field_name: str = "value", min_value: int = 1) -> int:
    ...
    @classmethod
    def validate_text_length(cls, value: str, field_name: str = "text", max_length: int = 1000, min_length: int = 1) -> str:
...
class MemoryValidator:
    ...
    def validate_memory_data(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'memory_id' in data:
            validated['memory_id'] = DataValidator.validate_uuid(data['memory_id'], 'memory_id')
        
        if 'namespace' in data:
            validated['namespace'] = DataValidator.validate_namespace(data['namespace'])
        
        if 'importance_score' in data:
            validated['importance_score'] = DataValidator.validate_importance_score(data['importance_score'])
        
        if 'category_primary' in data:
            validated['category_primary'] = DataValidator.validate_memory_category(data['category_primary'])
        
        if 'retention_type' in data:
            validated['retention_type'] = DataValidator.validate_retention_type(data['retention_type'])
    ...
    def validate_chat_data(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validated['user_input'] = DataValidator.sanitize_input(data['user_input'], 'user_input')
        validated['ai_output'] = DataValidator.sanitize_input(data['ai_output'], 'ai_output')
        validated['model'] = DataValidator.validate_text_length(data['model'], 'model', max_length=100)
        ...
        if 'tokens_used' in data:
            validated['tokens_used'] = DataValidator.validate_positive_integer(data['tokens_used'], 'tokens_used', min_value=0)
        
        if 'metadata' in data:
            validated['metadata'] = DataValidator.validate_json_dict(data['metadata'], 'metadata')
