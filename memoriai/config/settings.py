...
class AgentSettings(BaseModel):
    ...
    openai_api_key: Optional[str] = Field(
        default=None,
        description="OpenAI API key for memory processing"
    )
    default_model: str = Field(
        default="gpt-4o",
        description="Default model for memory processing"
    )
...
class MemorySettings(BaseModel):
    ...
    auto_cleanup: bool = Field(
        default=True,
        description="Enable automatic cleanup of expired memories"
    )
    ...
    max_short_term_memories: int = Field(
        default=1000,
        ge=10,
        description="Maximum number of short-term memories"
    )
    max_long_term_memories: int = Field(
        default=10000,
        ge=100,
        description="Maximum number of long-term memories"
    )
    context_injection: bool = Field(
        default=True,
        description="Enable context injection in conversations"
    )
...
class IntegrationSettings(BaseModel):
    ...
    litellm_enabled: bool = Field(
        default=True,
        description="Enable LiteLLM integration"
    )
    openai_wrapper_enabled: bool = Field(
        default=False,
        description="Enable OpenAI wrapper integration"
    )
    anthropic_wrapper_enabled: bool = Field(
        default=False,
        description="Enable Anthropic wrapper integration"
    )
    auto_enable_on_import: bool = Field(
        default=False,
        description="Automatically enable integrations on import"
    )
...
class MemoriSettings(BaseModel):
    ...
    def is_production(self) -> bool:
        ...
        return not self.debug and self.logging.level in [LogLevel.INFO, LogLevel.WARNING, LogLevel.ERROR]