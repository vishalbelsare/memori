...
def chat_with_memory(user_input: str, model: str = "n/a") -> str:
    ...
    try:
        ...
        if len(user_input.strip()) > 5:  # Only search for substantial queries
            try:
                context_result = memory_tool.execute(query=user_input[:100])  # Search with truncated query
                if context_result and "No relevant memories found" not in context_result:
...
print(f"- Memory database: digital_ocean_memory.db") 
print(f"- Namespace: digital_ocean_support")
...
if conversation_count > 2:
    try:
        ...
        print("✅ Memory analysis complete - key insights saved for future interactions!")
    except Exception as e:
