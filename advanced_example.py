...
import os
from memoriai import Memori, MemoryTool, create_memory_tool, record_conversation
...
def demonstrate_auto_recording():
    ...
    try:
        ...
        response = completion(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "user", "content": "What are Python decorators and how do they work?"}
            ]
        )
...
def demonstrate_memory_tool(memori_instance):
    ...
    context_result = memory_tool.execute(
        action="retrieve",
        query="Python optimization performance",
        limit=3
    )
